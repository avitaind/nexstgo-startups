...
if ( !function_exists( 'getleads_widget_init' ) ) {
	...
	function getleads_widget_init() {
		if ( function_exists( 'register_sidebar' ) ) {
			...
			register_sidebar(
			array(
				'name'			 => esc_html__( 'Woocommerce Widget Area', 'getleads' ),
				'id'			 => 'sidebar-2',
				'description'	 => esc_html__( 'Appears on posts and pages.', 'getleads' ),
				'before_widget'	 => '<div id="%1$s" class="widget %2$s">',
				'after_widget'	 => '</div> <!-- end widget -->',
...
if ( !function_exists( '_action_getleads_print_google_fonts_link' ) ) :
	...
	function _action_getleads_print_google_fonts_link() {
		...
		if ( $google_fonts_link != '' && defined( 'FW' ) ) {
			echo getleads_return( $google_fonts_link );
		} else {
			echo "<link href='https://fonts.googleapis.com/css?family=Open+Sans:400,300,300italic,400italic,500,700,700italic' rel='stylesheet'> \n";
		}
	}

	add_action( 'wp_head', '_action_getleads_print_google_fonts_link' );
endif;
...
function seopress_before_main_content() {
	echo '<div class="container">';
}

add_action( 'woocommerce_before_main_content', 'seopress_before_main_content' );

function seopress_after_main_content() {
	echo '</div>';
}

add_action( 'woocommerce_after_main_content', 'seopress_after_main_content' );
