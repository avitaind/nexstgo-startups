...
if ( isset( $atts[ 'background_options' ][ 'video' ][ 'video_type' ] ) && $bg_options == 'video' ) {
	$video_type	 = $atts[ 'background_options' ][ 'video' ][ 'video_type' ];
	$video_img	 = '';
	if ( $video_type[ 'select_video_xs' ] == 'media_upload' ) {
	...
	$bg_video_data_attr .= 'data-wallpaper-options="' . fw_htmlspecialchars( json_encode( array( 'source' => array( $filetype => $video_url, 'poster' => $video_img ) ) ) ) . '"';
